  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <input id="name" name="name" type="text" placeholder="John Doe" className="input-field" required autoComplete="name" />
        {state?.errors?.name && (
          <p className="mt-1 text-[11px]" style={{ color: 'var(--error)' }}>{state.errors.name[0]}</p>
        )}
      </div>
      ...
      <div>
        ...
        <input id="password" name="password" type="password" placeholder="••••••••" className="input-field" required autoComplete="new-password" />
        {state?.errors?.password && (
          <div className="mt-2 space-y-0.5">
            {state.errors.password.map((error: string) => (
              <p key={error} className="text-[11px] flex items-center gap-1.5" style={{ color: 'var(--error)' }}>
                <span className="w-1 h-1 rounded-full shrink-0" style={{ background: 'var(--error)' }} />
                {error}
              </p>
            ))}
          </div>
